...
start_time = dt.datetime(2015, 7, 1)
...
data.plot(subplots=True, grid=True, figsize=(8, 6), title=['CME Lean hogs future daily price','South Korea Lean hogs future daily price'])

#get returns
rets = np.log(data / data.shift(1)) * 100

#plot the returns
rets.plot(subplots=True, grid=True, figsize=(8, 6), title=['CME Lean hogs future log return * 100','South Korea Lean hogs future log return * 100'])

xdat = rets['LeanHogs']['2016':'2019']
y1dat = rets['KR']['2016':'2019']
#y2dat = rets['EE']

result = sm.ols(formula="LeanHogs ~ KR", data=rets['2016':'2019'][:-30]).fit()
print(result.params)
...
pred = result.params[0] + result.params[1] * rets['KR']['2016':'2019'][-30:]
plt.plot(rets['LeanHogs']['2016':'2019'][-50:].dropna())
plt.plot(pred)
plt.title('OLS model 30 days predication')

mse = ((rets['LeanHogs']['2016':'2019'][-30:] - pred) ** 2).mean()
